...
object WebsocketHandshake {
  ...
  class ClientHandshaker(host: String) {
    ...
    def checkResponse(headers: Traversable[(String, String)]): Either[String, Unit] = {
      ...
      } else headers.find{ case (k, v) => k.equalsIgnoreCase("Sec-WebSocket-Accept") }.map {
  ...
  private[WebsocketHandshake] val magicString = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11".getBytes(US_ASCII)
