...
object FrameTranscoder {
  // Masks for extracting fields
  private val OP_CODE = WebsocketBits.OP_CODE
  private val FINISHED = WebsocketBits.FINISHED
  private val MASK = WebsocketBits.MASK
  private val LENGTH = WebsocketBits.LENGTH

  // message op codes
  private val CLOSE = WebsocketBits.CLOSE
  private val PING = WebsocketBits.PING
  private val PONG = WebsocketBits.PONG
...
class FrameTranscoder(val isClient: Boolean) {
  def frameToBuffer(in: WebsocketBits.WebSocketFrame): Array[ByteBuffer] = {
    var size = 2
    ...
    if (in.length > 125 && (opcode == FrameTranscoder.PING || opcode == FrameTranscoder.PONG || opcode == FrameTranscoder.CLOSE))
    ...
    val b1 = if (in.last) opcode.toByte | FrameTranscoder.FINISHED else opcode.toByte
    ...
    if (isClient) b2 = FrameTranscoder.MASK.toByte
    ...
    if (isClient && in.length > 0) { // need to mask outgoing bytes
      ...
      val maskBits = Array(((mask >>> 24) & 0xff).toByte, ((mask >>> 16) & 0xff).toByte, ((mask >>> 8) & 0xff).toByte, ((mask >>> 0) & 0xff).toByte)
      ...
      for (i <- 0 until in.length) {
        buff.put((data(i) ^ maskBits(i & 0x3)).toByte) // i & 0x3 is the same as i % 4 but faster
      }
      buff.flip
      Array[ByteBuffer](buff)
    } else {
      buff.flip
      Array[ByteBuffer](buff, ByteBuffer.wrap(in.data))
    }
  }
  ...
  /** Method that decodes ByteBuffers to objects. None reflects not enough data to decode a message
    ...
    */
  def bufferToFrame(in: ByteBuffer): WebsocketBits.WebSocketFrame = {

    if (in.remaining < 2 || FrameTranscoder.getMsgLength(in) < 0) {
      null
    } else {
      val opcode = in.get(0) & FrameTranscoder.OP_CODE
      val finished = (in.get(0) & FrameTranscoder.FINISHED) != 0
      val masked = (in.get(1) & FrameTranscoder.MASK) != 0

      if (masked && isClient) throw new FrameTranscoder.TranscodeError("Client received a masked message")
      ...
      WebsocketBits.makeFrame(opcode, FrameTranscoder.decodeBinary(slice, m), finished)
    }
  }
}
